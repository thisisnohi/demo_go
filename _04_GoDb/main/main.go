...
func init() {
	...
	sqlStr := "root:nohi1234@tcp(10.0.0.210:3306)/go_test?charset=utf8&parseTime=true&loc=Local"
	var err error
...
func getMulData(c *gin.Context) {
	...
	sqlStr := "select name,age from user where address like ?"
	fmt.Println("请求参数：" + address + "")
	address = "%" + address + "%"
	fmt.Println("请求参数：" + address)
...
func getData(c *gin.Context) {
	name := c.Query("name")
	sqlStr := "select age,address from user where name like ?"
	var u SqlUser
